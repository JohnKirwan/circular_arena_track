...
```
Modify some conditions and show the first rows.
...
df$heading_rad <- mapply(rad_unclock, df$heading_rad)
...
get_animal_pos <- function(trial_limits,animal_track){
 ...
 while (i <= dim(trial_limits)[1]){    # for each trial in a video 
  ...
  } else if(any(XY$trans_rho<circ0) & # if points either side of circles but not between
        any(XY$trans_rho>circ1) & !any(XY$trans_rho>circ0 & XY$trans_rho<circ1)){
  trial_limits$x0[i] <- last(XY$trans_x[XY$trans_rho<circ0]) #get last pts
  trial_limits$y0[i] <- last(XY$trans_y[XY$trans_rho<circ0]) #in inner circ
  } else{
  ...
  if(any(XY$trans_rho <= circ1) & any(XY$trans_rho > circ1)){ # if pts both sides of outer circ
    trial_limits$x1[i] <- XY$trans_x[XY$trans_rho >= circ1][1] #first pts beyond
    trial_limits$y1[i] <- XY$trans_y[XY$trans_rho >= circ1][1] # outer circle
  }else if(!any(XY$trans_rho > circ1)){
...
```{r}
trackz_tbl     <- subset(trackz_tbl, arc %in% c(0,17,40))
...
trackz_tbl %>% #filter(obstacle=="obst") %>% 
 ggplot() + coord_fixed() +
 geom_path(aes(x=trans_x,y=trans_y,group=vid_ord,color=trial_set),
             size=.5)  +
 geom_point(aes(x=trans_x, y=trans_y,group=vid_ord,color=trial_set),
             size=.5) + 
...
```{r rel headings fig, fig.height = 8, fig.width = 7}
how_many <- 50
...
trackz_tbl %>%  #filter(obstacle=="obst") %>% 
 ...
 ggplot() + coord_fixed() + 
 geom_path(aes(x=rel_x,y=rel_y,group=vid_ord,color=succ5))+
 geom_point(aes(x=rel_x,y=rel_y,group=vid_ord,color=succ5),size=.3)+ #,size=succ5>0
 ...
 # trialz$obstacle=="obst" & as.numeric(trialz$id) <= how_many,],
   aes(x=cos(radians),y=sin(radians),color=succ5),size=5,shape=1)+ #alpha=.5,
 geom_segment(data=trialz,#[
   #trialz$obstacle=="obst" & as.numeric(trialz$id) <= how_many,],
   aes(x=rel_x0,y=rel_y0,xend=cos(radians),yend=sin(radians),
     colour=succ5),linetype="dotted") +
...
headings_summ %>% pivot_longer(c(quart,quint,sext)) %>% filter(type!="bar") %>%
...
#### TROUBLESHOOTING TRIAL NUMBER

There are four trials in every video according to the spreadsheet but this is not true fo the files.

```{r}
any(table(df$video) != 4)
table(trialz$video)
```

For instance, there are two full sets for 3993

```{r}
trialz$quadrant[trialz$video=="GOPR3993"]
```
We can partly check by the angles

```{r}
round((pi/2)-trialz$radians[trialz$video=="GOPR3993"],2)
```
