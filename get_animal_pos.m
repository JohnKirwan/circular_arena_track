...
%   % to include origin
    ...
    while h <= length(animal_track(:,1))                      % cycle through the frames
    j   = animal_track(h,1);                                   % pick the frame
    
        if j >= trial_limits(i,1) && j <= trial_limits(i,6)       % if in the bounds of a given trial
            track_x = [track_x animal_track(h,2)];                % append x values for each trial
            track_y = [track_y animal_track(h,3)];                % append y values for each trial
        end
