...
get_trial_limits <- function(landmarks,animal_track,file_tbl,vid_num){ #,
 ...
 trial_limits <- as_tibble(array(0,dim=c(num_trials,16),
    ...
    "in_theta","in_rho","out_theta","out_rho"))),
    .name_repair="unique")
...
get_animal_pos <- function(trial_limits,animal_track){
 ...
 while (i <= dim(trial_limits)[1]){    # for each trial in a video 
   ...
   trial_limits[i,13] <- XY$trans_theta[which.min(abs(XY$trans_rho - .25))]
   trial_limits[i,14] <- XY$trans_rho  [which.min(abs(XY$trans_rho - .25))]
   trial_limits[i,15] <- XY$trans_theta[which.min(abs(XY$trans_rho - .5))]
   trial_limits[i,16] <- XY$trans_rho  [which.min(abs(XY$trans_rho - .5))]
   i = i + 1
 }
...
Then make a function which transforms the coords from all tibbles in the big list
