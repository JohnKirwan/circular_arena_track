 ...
 while (i <= num_trials){ # for each trial
  ...
  trial_limits[i,4:5] <- as.list(as.numeric(landmarks[i*2,2:3]))# trial i stim centre x,y
     if (i < num_trials){
       ...
       j =  max(animal_track[,1]) # max along dimension 1 
     ...
     #theta1 = theta1 + pi/2; # set to convent. degree values with 0 at top and clockwise
     if (theta1 < -pi){        
         theta1 = theta1 + pi*2;     
     }else if (theta1 > pi){   
         theta1 = theta1 - pi*2     
...
get_animal_pos <- function(trial_limits,animal_track){
 ...
 while (i <= dim(trial_limits)[1]){    # for each trial in a video 
   ...
   trial_limits[i,17] <- paste0(trial_limits[i,12],"_",i)
   i = i + 1
 }
...
trackz_tbl %>% 
  ggplot() + coord_fixed() +
  geom_path(aes(x=trans_rho*cos(trans_theta),y=trans_rho*sin(trans_theta),
  ...
  geom_point(data=all_trial_limits, aes(x=1*cos(radians),y=1*sin(radians)),
             alpha=0.1, size=3) +
...
df %>% 
  ...
  annotate("path", color="orange",
  x=0+1*cos(seq(0,2*pi,length.out=100)),y=0+1*sin(seq(0,2*pi,length.out=100))) +
  geom_point(data=df, aes(x=1*cos(pi/2-heading_rad),y=1*sin(pi/2-heading_rad)),
             alpha=0.1, size=3) +
  xlab('') + ylab('') + theme_classic() + ggtitle('Plot all tracks simultaneously') +
  theme(axis.line=element_blank(),axis.ticks=element_blank(),
        axis.text=element_blank()) + theme(legend.position = "none") + 
