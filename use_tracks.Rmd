...
linecirc <- function(in_x,in_y,out_x,out_y){
  ...
  dist2track    <- c(0,0) 
  dist2track[1] <- sqrt( (ints[1,1]-out_x)^2 + (ints[1,2]-out_y)^2 )
  dist2track[2] <- sqrt( (ints[2,1]-out_x)^2 + (ints[2,2]-out_y)^2 )
...
trackz_tbl %>% #group_by(video)
  ggplot() + # geom_circle(aes(x0=0,y0=0,r=1)) +
  coord_fixed() +
  geom_point(aes(x=trans_rho*cos(trans_theta),y=trans_rho*sin(trans_rho))) + annotate("path", 
   x=0+1*cos(seq(0,2*pi,length.out=100)),
   y=0+1*sin(seq(0,2*pi,length.out=100)))
# trans_rho*cos(trans_theta),y0=trans_rho*sin(trans_rho)
```
