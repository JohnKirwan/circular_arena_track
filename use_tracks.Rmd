...
# geom_path is already facing up, without (pi/2)- , geom_point() is not
JEB18_tracks %>% filter(cond=="DoG 40") %>% #filter(video==vid) %>%
  ggplot() + coord_fixed() + 
  ...
  geom_point(data=all_trial_limits[
    all_trial_limits$cond=="DoG 40",],#[all_trial_limits$video==vid,],
             aes(x=cos(radians),y=sin(radians),
             shape=1),alpha=.2,size=3) + scale_shape_identity() +
  geom_point(data=df[df$cond=="DoG 40",],#[df$video==vid,], # data from the paper
...
```{r Headings in paper fig, fig.height = 7, fig.width = 7}
