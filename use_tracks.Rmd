...
```{r}
intersect_angle1 <- array(numeric(), c(dim(trial_limits)[1],1))    
intersect_angle2 <- array(numeric(), c(dim(trial_limits)[1],1)) 
which_intersect  <- array(numeric(), c(dim(trial_limits)[1],1)) 
```
...
linecirc <- function(in_x,in_y,out_x,out_y){
  ...
  int_1   <- unlist(c(t1*(out_x - in_x) + in_x,t2*(out_y - in_y) + in_y))
  int_2   <- unlist(c(t1*(out_x - in_x) + in_x,t2*(out_y - in_y) + in_y))

  d1 <- sqrt(int_1[1]^2 + int_1[2]^2) # why are these not 1?
  d2 <- sqrt(int_2[1]^2 + int_2[2]^2) 
  return(c(t1,t2))
...
```{r}
linecirc(all_trial_limits[1,13],all_trial_limits[1,14],
         all_trial_limits[1,15],all_trial_limits[1,16])
```
