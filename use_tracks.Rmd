...
Create a fx cart2pol to generate polar coordinates. 
...
```{r}
write_csv(map_dfr(all_trackz, ~ .x),  file = "tracks_file.csv")
write_delim(map_dfr(all_trackz, ~ .x),file = "tracks_file.txt",delim = "\t")
```
...
```{r}
linecirc <- function(in_x,in_y,out_x,out_y){
  # at^2+bt+c=0
  h = 0 # circle origin x
  k = 0 # circle origin y
  radius = 1
    
  a = (out_x - in_x)^2 + (out_y - in_y)^2 # vector length
  b = (2*(out_x - in_x)*(in_x - h)) + (2*(out_y - in_y)*(in_y - k)) # 
  c = (in_x - h)^2 + (in_y - k)^2 - radius^2
  D <- (b^2) - (4*a*c) # discriminant - must be positive
  if(a <= 0){warning("Vector length is not positive!")}
  if(D <= 0){warning("Discriminant is not positive!")}
  ...
  ints <- rbind(unlist(c( t1*(out_x - in_x) + in_x, t1*(out_y - in_y) + in_y)),
            unlist(c( t2*(out_x - in_x) + in_x, t2*(out_y - in_y) + in_y)))
            
  int_2 <- unlist(c( t2*(out_x - in_x) + in_x, t2*(out_y - in_y) + in_y))

  d <- c(sqrt(ints[1,1]^2 + ints[1,2]^2),sqrt(ints[2,1]^2 + ints[2,2]^2))
  if(all.equal(d[1],d[2])==FALSE){warning("Intersect is off unit circle!")}
  
  dist2track    <- c(0,0) 
  dist2track[1] <- sqrt( (int_1[1]-out_x)^2 + (int_1[2]-out_y)^2 )
  dist2track[2] <- sqrt( (int_2[2]-out_x)^2 + (int_2[2]-out_y)^2 )
...
all_trial_limits[,17:18] <- pmap_dfr(list(
  all_trial_limits$in_x,all_trial_limits$in_y,
                    all_trial_limits$out_x,all_trial_limits$out_y), linecirc)
```


```{r}
#all_trial_limits[,19]
atan2(all_trial_limits[[1,18]],all_trial_limits[[1, 17]])
```
...
for(i = 1:num_trials){
    ...
    animal_vector[i, 1] = inner_x ;  
    animal_vector[i, 2] = inner_y ;
    animal_vector[i, 3] = outer_x ;  
    animal_vector[i, 4] = outer_y ;
