...
if(any(files_df$track    %in% list.files(normalizePath(folder)) == FALSE)){
  stop(print(paste0("The following track file is missing:",
    files_df$track[which(files_df$track %in% list.files(normalizePath(folder)) == FALSE)]
                    )))
}
...
if(any(list.files(normalizePath(folder))[grep('*point02.txt$',list.files(
           normalizePath(folder)))] %in% files_df$track == FALSE)){
...
get_trial_limits <- function(landmarks,animal_track,file_tbl){ #,
...
#trial_limits = zeros([num_trials 11]); # pick out start frame and centre and stimulus coords
  trial_limits <- array(dim=c(num_trials,12)) 
...
#  trial_limits %>% mutate(across(.cols = everything(),as.numeric))
  cols(trial_limits = col_number()) #make all numeric
  i = 1 
 ...
 while (i <= num_trials){ # for each trial
    ...
    print(file_tbl[i,1])
    trial_limits[i, 12]      <- file_tbl[[i,1]];
    i = i + 1 
 }
 return(trial_limits)
}
...
```{r}
trial_limits <- get_trial_limits(animal_landmark,animal_track,files_df) #
```
