...
files_df <-tibble(basename=sub("*_point01.txt","",#subset str before exp
...
if(any(!files_df$track %in% list.files(normalizePath(folder),recursive = TRUE))){
  stop(print(paste0("The following track file is missing:",
    files_df$track[which(!files_df$track %in% list.files(
...
if(any(!list.files(normalizePath(folder),recursive = TRUE)[grep('*point02.txt$',
    list.files(normalizePath(folder),recursive = TRUE))] %in% files_df$track)){
...
get_animal_pos <- function(trial_limits,animal_track){
 ...
 while (i <= dim(trial_limits)[1]){    # for each trial in a video 
   ...
   trial_limits$x1[i] <- XY$trans_x[which.min(abs(XY$trans_rho - .5))] #.5
...
trackz_tbl %>% group_by(cond) %>% #filter(video=="GOPR3997") %>%
  ggplot() + coord_fixed() +
  geom_path(aes(x=trans_x, y=trans_y,group=vid_ord),
             alpha=.5, size=.75) + 
  geom_point(aes(x=trans_x, y=trans_y,group=vid_ord),
             alpha=.05, size=.5) + 
...
# geom_path is already facing up, without (pi/2)- , geom_point() is not
trackz_tbl %>%  #  %>% filter(cond=="DoG 40") 
  ggplot() + coord_fixed() + 
  geom_path(aes(x=rel_x, y=rel_y, group=vid_ord),alpha=.5,size=.2) + 
  geom_point(aes(x=rel_x, y=rel_y, group=vid_ord),alpha=.5,size=.5) + 
  facet_wrap(vars(cond)) +
  annotate("path", color="orange",
  ...
  geom_point(data=all_trial_limits,
   aes(x=cos(radians),y=sin(radians),shape=1),alpha=.2,size=3)+scale_shape_identity() +
   xlab('') + ylab('') + theme_classic() + ggtitle('Plot all tracks simultaneously') +
  theme(axis.line=element_blank(),axis.ticks=element_blank(),
        axis.text=element_blank()) + theme(legend.position = "none")
```
...
write_csv(all_trial_limits,  file = "headings_file.csv")
write_delim(all_trial_limits,file = "headings_file.txt",delim = "\t")
